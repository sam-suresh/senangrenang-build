...
function submit_check_first(){
		...
		$.ajax({
			    type: 'POST',
			    url: 'http://user.mydroid.io/sam2/front_end/api_registration.php',
			    data: {
...
function submit_to_joomla2(name,username,password1,password2,email1,email2,option,task){
		$.ajax({
			    type: 'POST',
			    url: 'http://user.mydroid.io/sam2/index.php', //url of joomla website
			    data: {
			    	'option': option,
			    	'task': task,
			    	'jform[name]':name,
...
//    START CONNECT TO JOOMLA BACKEND
	...
	function submit_to_joomla(){ 
		var jform_username = document.getElementById("username1").value;
		...
		$.ajax({
                type: "POST",
                url: "http://user.mydroid.io/sam2/front_end/api_login.php",
                data: {
                    username: jform_username,
...
//    END CONNECT TO JOOMLA BACKEND
     ...
     $( document ).ready(function() {
			    $.ajax({
				    url: 'http://user.mydroid.io/sam2/api.php?private_key=e4ed5acb3a53e3935a1a8f1676fb111de2a4842a',
				    type: "get",
				    dataType: "json",
				    success: function(response) {
    ...
    $(document).on("click", "#sendfeedback", function(evt)
    {
      ...
      var $form = $( this ),
      url = "http://user.mydroid.io/sam2/sam.php";
